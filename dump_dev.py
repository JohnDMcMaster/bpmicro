...
if __name__ == "__main__":
    ...
    if cmd.sm_is_inserted(gpio):
        ...
        if args.save:
            open(os.path.join(dout, 'sm_eeprom_%s.bin' % sm.name),
                 'w').write(sm_eeprom)
        ...
        if args.save:
            open(os.path.join(dout, 'sm_eeprom2_%s.bin' % sm.name),
                 'w').write(sm2_eeprom)
    else:
    ...
    if args.save:
        open(os.path.join(dout, 'ta_eeprom_%s.bin' % ta.name),
             'w').write(ta_eeprom)

    print()
    print('ADC')
    print('NOTE: best guess')
    for name, (reg, sf) in cmd.adcs.items():
